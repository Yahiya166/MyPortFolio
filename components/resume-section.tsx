    ...
    <div className="space-y-8 md:space-y-10">
      ...
      {'certifications' in data && data.certifications && (
        <div>
          ...
          <div className="space-y-4">
            {data.certifications.map((item: { title: string; period: string; description: string }, index: number) => (
              <div key={index} className="relative pl-5 md:pl-6 pb-6 border-l-2 border-border last:pb-0">
                <div className="absolute -left-[9px] top-0 w-4 h-4 rounded-full bg-accent" />
                <h4 className="text-base md:text-lg font-semibold text-foreground mb-2">{item.title}</h4>
                <p className="text-xs md:text-sm text-accent mb-2">{item.period}</p>
          ...
          {data.experience.map((item, index) => (
            <div key={index} className="relative pl-5 md:pl-6 pb-6 border-l-2 border-border last:pb-0">
              ...
              {'company' in item && item.company && (
                <p className="text-sm text-foreground/80 mb-1 font-medium">{item.company}</p>
              )}
              <p className="text-xs md:text-sm text-accent mb-2">{item.period}</p>
              <p className="text-xs md:text-sm text-muted-foreground leading-relaxed">{item.description}</p>
            </div>
